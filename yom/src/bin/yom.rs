...
use yara_on_map::metrics::{Registry, serve_http};
...
#[derive(Parser)]
#[command(name="yom", version="0.1.0", about="YARA-on-Map: page-wise memory scanner for Windows")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd
}
...
fn write_jsonl(out: &Option<PathBuf>, fs: &[Finding]) {
    if let Some(p) = out {
        if let Ok(mut f) = fs_extra::file::OpenOptions::new().create(true).append(true).open(p) {
            for x in fs {
...
fn main() -> Result<()> {
    ...
    let cli = Cli::parse();
    match cli.cmd {
        Cmd::Run { config, rules, pids, duration, listen, jsonl, enforce } => {
            let mut cfg: AppCfg = if config.exists() {
                serde_yaml::from_str(&std::fs::read_to_string(&config)?)?
            } else { AppCfg::default() };
            cfg.engine.yara_path = cfg.engine.yara_path.clone();
            let eng = YaraExternal::new(cfg.engine.yara_path.clone(), rules.clone(), cfg.scan.timeout_ms)?;
            ...
            loop {
                ...
                let tasks = build_tasks(if pid_list.is_empty(){None}else{Some(pid_list)}, &cfg.scan.priorities);
                ...
                if duration == 0 { break; }
                if start.elapsed().as_secs() >= duration { break; }
        ...
        Cmd::Replay { file, rules } => {
            ...
            let pid = if data["pid"].as_str() == Some("self") { std::process::id() } else { data["pid"].as_u64().unwrap_or(0) as u32 };
            ...
            if let Some(arr) = data["pages"].as_array() {
                for it in arr {
                    let base = u64::from_str_radix(it["base"].as_str().unwrap().trim_start_matches("0x"), 16).unwrap();
                        ...
                        Ok(hits) if !hits.is_empty() => {
                            for rule in hits {
                                findings.push(yara_on_map::types::Finding{
                                    pid, base, size: bytes.len(), rule: rule.clone(),
                                    severity: "high".into(), message: format!("match {} at 0x{:x}", rule, base),
                                });
                            }
                        }
                        _ => {}
